...
class LLMSectionDetector:
    """Use LLM to detect document sections/headings more intelligently"""

    def __init__(self, client, model="gpt-4.1"):
        self.client = client
        self.model = model

    def detect_sections(self, paragraphs: list) -> list:
        """
        Input: list of paragraph strings
        Output: list of (is_heading: bool, text: str, confidence: float)
        """
        if not paragraphs:
            return []
        ...
        for i in range(0, len(paragraphs), batch_size):
            ...
            try:
                ...
                batch_results = self._parse_llm_response(answer, batch)
                results.extend(batch_results)
            except Exception as e:
                print(f"LLM section detection failed: {e}")
                # Fallback to rule-based
                results.extend([(False, text, 0.5) for text in batch])

        return results

    def _build_prompt(self, paragraphs):
        prompt = """You are an expert scientific paper analyst.
Determine whether each paragraph is a **real section heading**.

**Good headings examples**: 
- "Synthesis", "General Procedure", "Example 1", "Experimental Section", 
- "Preparation of Compound 7a", "Results and Discussion"

**Bad examples (not headings)**:
- "synthesis", "experimental", chemical formulas like "HO CHO", "O O N N", "S1", "SOLVENT", short fragments.

Rules:
- Headings are usually short and have title-like structure.
- Reject pure chemical formulas and single lowercase words.
- Only return real section titles.

Return strict JSON:
{
  "sections": [
    {"index": 0, "is_heading": true, "confidence": 0.92},
    ...
  ]
}
...
"""
    ...
    def _parse_llm_response(self, response_text, original_paragraphs):
        try:
            import json
            ...
            return [
                (
                    ...
                    original_paragraphs[item.get("index", i)],
            ...
            # Fallback
            return [(False, p, 0.5) for p in original_paragraphs]
...
class PDF_text:
    ...
    def digital_pdf_para_cleaning(self, raw_para_in_pages):
        '''Digital PDF processing with balanced section detection'''
        # Step 1: Remove footers
        ...
        # Step 2: Merge broken paragraphs
        ...
        # Step 4: Initial split by ALL CAPS (Rule-based)
        subtitle_split_para_in_pages = []
        for page in no_figure_para_in_pages:
            splited_para_in_page = []
            for para in page:
                if para.strip():
                    splited_para = self.split_by_all_caps_sections(para)
                    splited_para_in_page.extend(splited_para)
            subtitle_split_para_in_pages.append(splited_para_in_page)

        # Step 5: Collect subtitles - 只剔除空字符串，其他全部保留
        cleaned_para_in_pages = []
        title_indices_in_page = []
        title_text_in_page = []

        for page in subtitle_split_para_in_pages:
            cleaned_para_this_page = []
            title_indices_this_page = []
            title_text_this_page = []

            for j, section in enumerate(page):
                subtitle_j = section[0] if isinstance(section, (list, tuple)) else ''
                para_j = section[1] if isinstance(section, (list, tuple)) else section

                subtitle_clean = subtitle_j.strip()

                # ==================== 极简规则：只剔除空字符串 ====================
                if subtitle_clean:                    # 非空就保留
                    cleaned_para_this_page.append(subtitle_j)
                    title_indices_this_page.append(len(cleaned_para_this_page) - 1)
                    title_text_this_page.append(subtitle_j)
                else:
                    cleaned_para_this_page.append(para_j)   # 空标题当作普通段落

                if not subtitle_clean:   # 如果是空标题，继续添加段落内容
    ...
    def is_valid_section_title(title: str) -> bool:
        """Balanced and practical section title filter"""
        ...
        # 1. 高优先级允许的关键词
        good_keywords = {
            'synthesis', 'example', 'examples', 'experimental', 'experiment',
            'method', 'methods', 'procedure', 'preparation', 'general',
            'compound', 'reaction', 'results', 'discussion', 'conclusion',
            'supporting', 'characterization', 'data', 'spectra'
        }

        if any(kw in lower_t for kw in good_keywords):
            return True

        # 2. 排除常见噪音
        bad_patterns = [
            r'^S\d+$', r'^Figure', r'^Scheme', r'^Table', r'^Entry',
            r'^SOLVENT', r'^NMR', r'^IR', r'^MS', r'^HRMS', r'^CHO', r'^CN'
        ]
        if any(re.search(pat, t, re.IGNORECASE) for pat in bad_patterns):
            return False

        # 3. 排除化学式片段
        if re.search(r'[A-Z][a-z]?\s+[A-Z][a-z]?', t) and len(re.findall(r'[A-Z]', t)) >= 3:
            if len(t) < 40:
                return False

        # 4. 长度限制
        if len(t) > 120 or len(t) < 4:
            return False

        # 5. 常见标题格式
        if t.isupper() or t.istitle() or (len(words) <= 10):
            return True
    ...
    @staticmethod
    @staticmethod
    def split_by_all_caps_sections(text, meaningful_threshold=0.4, min_alpha_len=6):
        '''Return sections as list of tuples: [(title, contents), ...]'''
        text = text.strip()
        if not text:
            return [('', '')]
        ...
        def is_meaningful_phrase(phrase, threshold=0.4):
        ...
        special_sections = [
            'Introduction', 'Results and Discussion', 'Conclusion', 'Conclusions',
            'References', 'Acknowledgements', 'Experimental Section', 'Experimental',
            'Methods', 'Synthesis', 'Example', 'Examples'
        ]

        # Pattern for ALL CAPS headings
        pattern = r'(?<!\w)([■●\-–—\s]*[A-Z][A-Z\s\-–—&]{3,})(?!\w)'
        all_caps_matches = list(re.finditer(pattern, text))

        filtered_matches = []

        # Step 1: ALL CAPS detection
        for match in all_caps_matches:
            phrase = match.group(1).strip()
            alpha_count = sum(c.isalpha() for c in phrase)
            if alpha_count >= min_alpha_len and is_meaningful_phrase(phrase, meaningful_threshold):
                filtered_matches.append(match)

        # Step 2: Special section keywords
        for title in sorted(special_sections, key=len, reverse=True):
            for m in re.finditer(rf'\b{re.escape(title)}\b', text, re.IGNORECASE):
                start, end = m.start(), m.end()
                if start <= 20 or end >= text_len - 20:  # Near beginning or end
                    class SimpleMatch:
                        ...
                        def group(self, idx=0): return self._g
        ...
        # Deduplicate and build sections
